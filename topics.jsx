    ...
    {view==='list' ? (
        ...
        {list.map((t,i)=>{ const {cols,devs}=resolve(t); const types=collectionTypes(cols);
          ...
          <div key={t.id} style={{display:'flex',alignItems:'center',gap:14,padding:'13px 16px',borderTop:i?'1px solid var(--line)':0}}>
            ...
            <div style={{width:190,flex:'none',minWidth:0}}>
              ...
              <div className="muted" style={{fontSize:11.5,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{t.items.toLocaleString()} items · updated {t.updated}</div>
            </div>
            <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',gap:6}}>
              <div style={{fontSize:12,color:'var(--ink-2)',lineHeight:1.45,display:'-webkit-box',WebkitLineClamp:1,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{t.desc}</div>
              <div style={{display:'flex',gap:16,fontSize:11.5,color:'var(--ink-3)',fontWeight:500}}>
                <span><b style={{color:'var(--ink-2)',fontWeight:700}}>{cols.length}</b> collections</span>
                <span><b style={{color:'var(--ink-2)',fontWeight:700}}>{devs.length}</b> devices</span>
                <span><b style={{color:'var(--ink-2)',fontWeight:700}}>{types.length}</b> file types</span>
              </div>
            </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,paddingBottom:8}}>
        {list.map(t=>{ const {cols,devs}=resolve(t); const types=collectionTypes(cols);
          return (
          <div key={t.id} className="card card-hover" style={{padding:'16px 16px 14px',display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div className="muted" style={{fontSize:11.5,marginTop:2,display:'flex',alignItems:'center',gap:7}}>
                  <span>{t.items.toLocaleString()} items</span>
                  {t.trend==='up' && <span style={{display:'inline-flex',alignItems:'center',gap:2,color:'#16A34A',fontWeight:600}}><Icon name="trend_up" size={12} sw={2.2}/>rising</span>}
                </div>
              </div>
              <FavStar on={!!favs[t.id]} onToggle={()=>toggleFav(t.id)}/>
            </div>
            <div style={{fontSize:12.5,color:'var(--ink-2)',lineHeight:1.5,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden',minHeight:'2.95em'}}>{t.desc}</div>
            <div style={{display:'flex',paddingTop:12,borderTop:'1px solid var(--line)'}}>
              {[['Collections',cols.length],['Devices',devs.length],['File types',types.length]].map(([lb,v],i)=>(
                <React.Fragment key={lb}>
                  {i>0 && <div style={{width:1,background:'var(--line)'}}></div>}
                  <div style={{flex:1,textAlign:'center',minWidth:0}}>
                    <div style={{fontSize:18,fontWeight:700,color:'var(--ink)',letterSpacing:'-.01em',fontVariantNumeric:'tabular-nums'}}>{v}</div>
                    <div style={{fontSize:10.5,color:'var(--ink-4)',fontWeight:500,marginTop:2}}>{lb}</div>
                  </div>
                </React.Fragment>
              ))}
            </div>
