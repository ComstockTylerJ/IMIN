    ...
    <div style={{position:'fixed',inset:0,zIndex:320,display:'flex',alignItems:'flex-start',justifyContent:'center',paddingTop:'8vh',paddingBottom:'4vh',overflowY:'auto'}}>
      ...
      <div className="pop card" style={{position:'relative',width:'min(540px,94vw)',boxShadow:'var(--shadow-lg)',borderRadius:16,overflow:'hidden'}}>
        <div style={{padding:'18px 22px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',gap:13}}>
          <span style={{width:40,height:40,borderRadius:11,background:'var(--secondary)',color:'var(--secondary-foreground)',display:'flex',alignItems:'center',justifyContent:'center',flex:'none'}}><Icon name={icon} size={20}/></span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:16,fontWeight:700,letterSpacing:'-.02em',color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{title}</div>
        ...
        <div style={{padding:'13px 22px',borderTop:'1px solid var(--border)',display:'flex',gap:10,background:'var(--background)'}}>
...
function InfoItem({icon, title, sub, badge, onClick}){
  return (
    <div onClick={onClick} style={{display:'flex',alignItems:'center',gap:11,padding:'8px',margin:'0 -8px',borderRadius:8,cursor:onClick?'pointer':'default',transition:'.12s'}}
      onMouseEnter={e=>{if(onClick)e.currentTarget.style.background='var(--background)';}}
      onMouseLeave={e=>{if(onClick)e.currentTarget.style.background='transparent';}}>
      <span style={{width:30,height:30,borderRadius:8,background:'var(--secondary)',color:'var(--secondary-foreground)',display:'flex',alignItems:'center',justifyContent:'center',flex:'none'}}><Icon name={icon} size={15}/></span>
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:13,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{title}</div>
...
function InfoRows({rows}){
  ...
  return (
    <div style={{display:'flex',flexDirection:'column'}}>
      {r.map(([k,v],i)=>(
        <div key={i} style={{display:'flex',justifyContent:'space-between',gap:16,padding:'8px 0',borderTop:i?'1px solid var(--border)':0,fontSize:13}}>
          <span style={{color:'var(--muted-foreground)',flex:'none'}}>{k}</span>
          <span style={{color:'var(--foreground)',fontWeight:500,textAlign:'right',minWidth:0,wordBreak:'break-word'}}>{v}</span>
        </div>
      ))}
    </div>
  );
...
function TypeChips({types}){
  return (
    <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
      {types.length?types.map(ty=><span key={ty} className="badge" style={{background:'var(--secondary)',color:'var(--secondary-foreground)',height:24,fontFamily:'ui-monospace,Menlo,monospace',fontWeight:600}}>{ty}</span>)
...
function MetaLine({label, value}){
  return (
    <div style={{display:'flex',gap:8,fontSize:11.5,lineHeight:1.45}}>
      ...
      <span style={{flex:1,minWidth:0,color:'var(--secondary-foreground)',fontWeight:500,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{value}</span>
    </div>
  );
}
...
function CardActions({onView, onDetails, viewLabel='View', viewIcon='arrow_right', stacked}){
  return (
    <div style={{display:'flex',gap:8, ...(stacked?{}:{marginTop:'auto'})}}>
      ...
      <button className="btn btn-ghost btn-sm" style={{border:'1px solid var(--border-strong)'}} onClick={e=>{e.stopPropagation();onDetails();}}>Details</button>
    </div>
  );
}
    ...
    {view==='list' ? (
      <div className="card" style={{padding:0,overflow:'hidden'}}>
        {list.map((t,i)=>{ const {cols,devs}=resolve(t); const types=collectionTypes(cols);
          return (
          <div key={t.id} style={{display:'flex',alignItems:'center',gap:14,padding:'13px 16px',borderTop:i?'1px solid var(--border)':0}}>
            ...
            <div style={{width:190,flex:'none',minWidth:0}}>
              <div style={{fontSize:13.5,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{t.name}</div>
        ...
        {list.map(t=>{ const {cols,devs}=resolve(t); const types=collectionTypes(cols);
          ...
          <div key={t.id} className="card card-hover" style={{padding:'15px 16px 14px',display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'flex',alignItems:'flex-start',gap:12}}>
              <span style={{width:40,height:40,borderRadius:11,background:'var(--secondary)',color:'var(--secondary-foreground)',display:'flex',alignItems:'center',justifyContent:'center',flex:'none'}}><Icon name={t.icon} size={20}/></span>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:14.5,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',letterSpacing:'-.01em'}}>{t.name}</div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:7,paddingTop:11,borderTop:'1px solid var(--border)'}}>
              <MetaLine label="Collections" value={cols.length?cols.map(c=>c.name).join(' · '):'None'}/>
    ...
    <InfoModal title={t.name} sub={t.desc} icon={t.icon} onClose={onClose} onView={onView} viewLabel="Open topic" viewIcon="layers">
      ...
      <InfoSection title="Contributors" count={ppl.length}>
        <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {ppl.map(p=><span key={p.id} style={{display:'flex',alignItems:'center',gap:7,fontSize:12.5,color:'var(--secondary-foreground)',background:'var(--background)',border:'1px solid var(--border)',borderRadius:999,padding:'3px 11px 3px 3px'}}><Avatar id={p.id} size={20} ring={false}/>{p.name}</span>)}
        </div>
      </InfoSection>
    </InfoModal>
  );
      ...
      <div style={{borderBottom:'1px solid var(--border)',background:'rgba(255,255,255,.65)',backdropFilter:'blur(4px)'}}>
        <div className="page" style={{paddingTop:16,paddingBottom:20}}>
          <div style={{display:'flex',alignItems:'center',gap:7,fontSize:12.5,color:'var(--muted-foreground)',fontWeight:500,marginBottom:16}}>
            ...
            <Icon name="chevron_right" size={13} style={{opacity:.6}}/>
            <span style={{color:'var(--secondary-foreground)'}}>{t.name}</span>
          </div>
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:20,flexWrap:'wrap'}}>
            <div style={{display:'flex',alignItems:'center',gap:18,minWidth:0}}>
              ...
              <div style={{minWidth:0}}>
                <h1 style={{fontSize:25,fontWeight:700,letterSpacing:'-.03em',margin:0,color:'var(--foreground)'}}>{t.name}</h1>
        ...
        <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) 340px',gap:20,alignItems:'start'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14}}>
              {[['Items',t.items,'layers','#0073E6'],['Collections',cols.length,'collections','#475569'],['Devices',devs.length,'phone','#16A34A'],['Contributors',ppl.length,'users','#B5851C']].map(([l,v,ic,c])=>(
                <div key={l} className="card card-pad" style={{display:'flex',alignItems:'center',gap:13}}>
                  ...
                  <div>
                    <div style={{fontSize:22,fontWeight:700,color:'var(--foreground)',letterSpacing:'-.02em',lineHeight:1}}>{v}</div>
              ...
              {devs.length===0 ? <div className="muted" style={{fontSize:13,padding:'8px 0'}}>No devices tagged to this topic.</div> : (
                <div style={{display:'flex',flexDirection:'column'}}>
                  {devs.map((d,i)=>{ const ty=DEVICE_TYPE[d.type], stt=DEVICE_STATUS[d.status]; return (
                    <div key={d.id} style={{borderTop:i?'1px solid var(--border)':0}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            <div className="card card-pad" style={{display:'flex',gap:12,alignItems:'flex-start'}}>
              ...
              <div>
                <div style={{fontSize:13,fontWeight:600,color:'var(--foreground)'}}>About this lens</div>
