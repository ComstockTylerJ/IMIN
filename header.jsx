...
function Header({page, setPage, onCreate, onSearch, openRun, dashRole, setDashRole, glyph='diamond', framing='codename', flat=false}){
  ...
  return (
    <header style={{position:'sticky',top:0,zIndex:100}}>
      <div style={{height:22,background:'var(--primary)',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',
        fontSize:10.5,fontWeight:700,letterSpacing:'.09em',textTransform:'uppercase',whiteSpace:'nowrap',overflow:'hidden'}}>
        This system contains attorney client privileged information
      </div>
      ...
      <div style={{width:'100%',height:'100%',padding:'0 28px',position:'relative',
        ...
        {setDashRole && <RoleSwitcher role={dashRole} setRole={setDashRole} compact/>}
        ...
        <CommandPaletteEntry/>
        <button className="btn btn-ghost btn-icon" title="Help & tutorials" data-tour="help" onClick={()=>window.dispatchEvent(new CustomEvent('mcaap-tour:open'))}><Icon name="help" size={21}/></button>        <button className={'btn btn-ghost btn-icon'+(page==='termbase'?' active':'')} title="Term base" data-tour="termbase" onClick={()=>setPage('termbase')} style={page==='termbase'?{background:'var(--blue-t)',color:'var(--blue)'}:undefined}><Icon name="book" size={21}/></button>
...
function AvatarMenu({setPage, close}){
  ...
  return (
    <div className="pop card" style={{position:'absolute',top:44,right:0,width:248,padding:6,boxShadow:'var(--shadow-lg)',borderRadius:12}}>
      ...
      {item('beaker','Labs', ()=>setPage('labs'), {badge:'Beta'})}
