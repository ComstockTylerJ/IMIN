...
function ContentWorkspace({w, setPage, openCreate, flash, onSearch, embed, openPerson, openDevice, openTopic, openEntity, onAISearch}){
  ...
  const go=()=>{ if(ai && onAISearch){ onAISearch(query); } else { onSearch&&onSearch(query); } };
        ...
        <div className="page" style={{paddingTop:24}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap',marginBottom:18}}>
            ...
            <div style={{display:'flex',gap:3,background:'#EEF1F6',padding:3,borderRadius:9}} data-tour="explore-browse">
              {[['Collections','collections'],['Topics','layers'],['Devices','device'],['Entities','users']].map(([b,ic])=>(
          ...
          {browse==='Entities' && <EntitiesGrid filter={filter} flash={flash} openEntity={openEntity} view={gv} favOnly={favOnly} favs={favs} toggleFav={toggleFav}/>}
        </div>
      </div>
    </div>
  );
...
function DeviceGrid({filter, flash, openDevice, view='card', favs={}, toggleFav=()=>{}, favOnly=false}){
  ...
  const list=DEVICES.filter(d=>(d.name.toLowerCase().includes(f) || d.ev.toLowerCase().includes(f) || (d.imei||'').toLowerCase().includes(f) || DEVICE_TYPE[d.type].label.toLowerCase().includes(f)) && (!favOnly || favs[d.id]));
  const [detail,setDetail]=React.useState(null);
      ...
      <div className="card" style={{padding:0,overflow:'hidden'}}>
        {list.map((d,i)=>{ const ty=DEVICE_TYPE[d.type], stt=DEVICE_STATUS[d.status];
          ...
          <div key={d.id} style={{display:'flex',alignItems:'center',gap:14,padding:'13px 16px',borderTop:i?'1px solid var(--line)':0}}>
            ...
            <div style={{width:214,flex:'none',minWidth:0}}>
              <div style={{fontSize:13,fontWeight:700,color:'var(--ink)',fontFamily:'ui-monospace,Menlo,monospace',letterSpacing:'-.01em',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}} title={'IMEI '+d.imei}>{d.imei}</div>
              <div className="muted" style={{fontSize:11.5,display:'flex',alignItems:'center',gap:6,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}><span style={{fontWeight:600,color:'var(--ink-2)'}}>{d.name}</span><span style={{width:3,height:3,borderRadius:'50%',background:'var(--ink-4)',flex:'none'}}></span>{ty.label}<span style={{width:3,height:3,borderRadius:'50%',background:'var(--ink-4)',flex:'none'}}></span><span style={{fontFamily:'ui-monospace,Menlo,monospace'}}>{d.ev}</span></div>
            </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:16,paddingBottom:8}}>
        {list.map(d=>{ const ty=DEVICE_TYPE[d.type], stt=DEVICE_STATUS[d.status];
          ...
          <div key={d.id} className="card card-hover card-pad" style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div>
              <div style={{fontSize:9.5,fontWeight:700,letterSpacing:'.08em',color:'var(--ink-4)',textTransform:'uppercase'}}>IMEI</div>
              <div style={{fontSize:14,fontWeight:700,color:'var(--ink)',fontFamily:'ui-monospace,Menlo,monospace',letterSpacing:'-.01em',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',marginTop:2}} title={'IMEI '+d.imei}>{d.imei}</div>
              <div className="muted" style={{fontSize:11.5,marginTop:4,display:'flex',alignItems:'center',gap:6}}>
            ...
            <div style={{fontSize:11.5,color:'var(--ink-3)',fontWeight:500,display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
              <span style={{fontFamily:'ui-monospace,Menlo,monospace',color:'var(--ink-3)'}}>{d.ev}</span><span style={{width:3,height:3,borderRadius:'50%',background:'var(--ink-4)'}}></span>
