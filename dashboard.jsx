...
const DASH_ROLES = [
  { id: 'investigator', label: 'Investigator', icon: 'target', desc: 'Live case intelligence & subject watch' },
  { id: 'ops', label: 'Operations Lead', icon: 'columns', desc: 'Requests, actions, team & favorites' },
];
...
function RoleSwitcher({ role, setRole, compact }) {
  ...
  const cur = DASH_ROLES.find((r) => r.id === role) || DASH_ROLES[0];
  const menu = open &&
    <div className="pop" style={{ position: 'absolute', top: 'calc(100% + 8px)', zIndex: 120, background: '#fff', border: '1px solid var(--line)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', overflow: 'hidden', padding: 5, ...(compact ? { left: '50%', transform: 'translateX(-50%)', width: 300 } : { left: 0, right: 0 }) }}>
      {DASH_ROLES.map((r) => {
        const on = r.id === role;
        return (
          <button key={r.id} onClick={() => { setRole(r.id); setOpen(false); }} style={{ display: 'flex', alignItems: 'center', gap: 11, width: '100%', padding: '10px 11px', borderRadius: 9, border: 0, background: on ? 'var(--surface-2)' : 'transparent', cursor: 'pointer', textAlign: 'left' }}
          onMouseEnter={(e) => { if (!on) e.currentTarget.style.background = 'var(--hover)'; }} onMouseLeave={(e) => { if (!on) e.currentTarget.style.background = 'transparent'; }}>
            <span style={{ width: 32, height: 32, borderRadius: 9, background: on ? 'var(--primary)' : 'var(--surface-2)', color: on ? '#fff' : 'var(--ink-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}><Icon name={r.icon} size={16} /></span>
            <span style={{ flex: 1, minWidth: 0 }}>
              <span style={{ display: 'block', fontSize: 13.5, fontWeight: 600, color: 'var(--ink)' }}>{r.label}</span>
              <span style={{ display: 'block', fontSize: 11.5, color: 'var(--ink-3)', marginTop: 1 }}>{r.desc}</span>
            </span>
            {on && <Icon name="check" size={16} sw={2.4} style={{ color: 'var(--primary)', flex: 'none' }} />}
          </button>);
      })}
    </div>;
  if (compact) {
    return (
      <div ref={ref} style={{ position: 'relative' }}>
        <button onClick={() => setOpen((o) => !o)} title="Switch dashboard role"
          style={{ display: 'flex', alignItems: 'center', gap: 8, height: 34, padding: '0 12px', borderRadius: 999, border: '1px solid var(--line-2)', background: open ? 'var(--surface-2)' : '#fff', cursor: 'pointer', boxShadow: 'var(--shadow-sm)' }}>
          <Icon name={cur.icon} size={15} style={{ color: 'var(--primary)', flex: 'none' }} />
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--ink-4)' }}>Role</span>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)', whiteSpace: 'nowrap' }}>{cur.label}</span>
          <Icon name="chevron_down" size={14} sw={2} style={{ color: 'var(--ink-3)', flex: 'none', transform: open ? 'rotate(180deg)' : 'none', transition: '.15s' }} />
        </button>
        {menu}
      </div>);
  }
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 12, border: '1px solid var(--line)', background: '#fff', cursor: 'pointer', boxShadow: 'var(--shadow-sm)', width: 250 }}>
        <span style={{ width: 30, height: 30, borderRadius: 8, background: 'var(--primary-tint)', color: 'var(--primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: 'none' }}><Icon name={cur.icon} size={16} /></span>
        <span style={{ flex: 1, minWidth: 0, textAlign: 'left' }}>
          <span style={{ display: 'block', fontSize: 9.5, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: 'var(--ink-4)' }}>Viewing as</span>
          <span style={{ display: 'block', fontSize: 13.5, fontWeight: 600, color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{cur.label}</span>
        </span>
        <Icon name="chevron_down" size={16} style={{ color: 'var(--ink-3)', flex: 'none', transform: open ? 'rotate(180deg)' : 'none', transition: '.15s' }} />
      </button>
      {menu}
    </div>);
}

function Dashboard({ setPage, openTask, openCreate, role = 'investigator' }) {
  const [showTeam, setShowTeam] = React.useState(false);
  return (
    <div className="rise">
      <DashHero openCreate={openCreate} setPage={setPage} openTeam={() => setShowTeam(true)} role={role} />
...
function Weather() {
  // Simulated live conditions — refreshes on an interval so the icon/temperature update.
  const CYCLE = [
    { k: 'cloud_sun', label: 'Partly cloudy', t: 63 },
    { k: 'sun', label: 'Sunny', t: 70 },
    { k: 'cloud', label: 'Overcast', t: 60 },
    { k: 'cloud_rain', label: 'Light rain', t: 56 },
  ];
  const [i, setI] = React.useState(0);
  const [t, setT] = React.useState(CYCLE[0].t);
  const [beat, setBeat] = React.useState(false);
  React.useEffect(() => {
    const id = setInterval(() => {
      setI((v) => {
        const n = (v + 1) % CYCLE.length;
        setT(CYCLE[n].t + Math.round(Math.random() * 2 - 1));
        return n;
      });
      setBeat(true); setTimeout(() => setBeat(false), 400);
    }, 6000);
    return () => clearInterval(id);
  }, []);
  const w = CYCLE[i];
  return (
    <span title={w.label + ' · San Francisco · updated just now'}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 6, cursor: 'default' }}>
      <Icon name={w.k} size={14} sw={1.9}
        style={{ transition: 'transform .4s cubic-bezier(.2,.8,.3,1)', transform: beat ? 'scale(1.18)' : 'none' }} />
      <span style={{ fontVariantNumeric: 'tabular-nums' }}>{t}°F</span>
    </span>
  );
}

function DashHero({ openCreate, setPage, openTeam, role }) {
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
          <div>
            <div className="muted" style={{ fontSize: 13, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ width: 1, height: 14, background: 'var(--line-2)', display: 'inline-block', margin: '0 1px' }}></span>
              <Weather />
            </div>
            ...
            <p className="sec" style={{ fontSize: 14.5, margin: '6px 0 0', maxWidth: 520 }}>
              {investigator ?
              <React.Fragment>Live watch is active — <b style={{ color: 'var(--ink)', fontWeight: 600 }}>{(typeof CMD_ALERTS !== 'undefined' ? CMD_ALERTS.length : 5)} priority alerts</b> in the last 24h and <b style={{ color: 'var(--ink)', fontWeight: 600 }}>{(typeof SUBJECTS !== 'undefined' ? SUBJECTS.filter((s) => s.risk === 'Critical').length : 3)} critical-risk subjects</b> on your watchlist.</React.Fragment> :
              <React.Fragment>You have <button className="inline-link" onClick={() => window.__openTasks && window.__openTasks({ view: 'board' })}>4 tasks</button> due today and <button className="inline-link" onClick={() => setPage('review')}>2 reviews</button> awaiting your sign-off.</React.Fragment>}
            </p>
          </div>
...
function ActionCenter({ setPage }) {
  ...
  { label: 'Memos', count: 4, icon: 'route', color: '#475569', tint: '#F1F5F9', sub: 'Awaiting your concurrence', go: 'memos', lab: 'auto_memos' },
