  ...
  return (
    <div className="card card-pad" style={{display:'flex',alignItems:'center',gap:13}}>
      ...
      <div style={{minWidth:0}}>
        <div style={{fontSize:21,fontWeight:700,color:accent||'var(--foreground)',letterSpacing:'-.02em',lineHeight:1,whiteSpace:'nowrap'}}>{value}</div>
...
function InfoRow({icon, label, value, mono, copyable, flash, last}){
  return (
    <div style={{display:'flex',alignItems:'center',gap:11,padding:'9px 0',borderTop:'1px solid var(--border)'}}>
      <Icon name={icon} size={15} style={{color:'var(--ink-4)',flex:'none'}}/>
      <span style={{fontSize:12.5,color:'var(--muted-foreground)',width:74,flex:'none'}}>{label}</span>
      <span style={{flex:1,minWidth:0,fontSize:12.5,color:'var(--foreground)',fontWeight:550,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',fontFamily:mono?'ui-monospace,Menlo,monospace':'inherit'}}>{value}</span>
...
function PersonProfile({id, setPage, openTask, openDevice, flash}){
  ...
  const actions = (
    <div style={{display:'flex',gap:9,flexWrap:'wrap'}}>
      ...
      <button className="btn btn-ghost btn-sm" style={{border:'1px solid var(--border-strong)',padding:'0 10px'}} onClick={()=>flash&&flash('More actions')}><Icon name="more" size={16}/></button>
    </div>
  );
    ...
    <div className="rise">
      ...
      <div style={{borderBottom:'1px solid var(--border)',background:'rgba(255,255,255,.65)',backdropFilter:'blur(4px)'}}>
        <div className="page" style={{paddingTop:16,paddingBottom:20}}>
          <div style={{display:'flex',alignItems:'center',gap:7,fontSize:12.5,color:'var(--muted-foreground)',fontWeight:500,marginBottom:16}}>
            ...
            <Icon name="chevron_right" size={13} style={{opacity:.6}}/>
            <span style={{color:'var(--secondary-foreground)'}}>{p.name}</span>
          </div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:18,minWidth:0}}>
              ...
              <div style={{minWidth:0}}>
                <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                  <h1 style={{fontSize:25,fontWeight:700,letterSpacing:'-.03em',margin:0,color:'var(--foreground)'}}>{p.name}</h1>
                ...
                <div style={{fontSize:14,color:'var(--secondary-foreground)',fontWeight:550,marginTop:5}}>{p.relType} · {p.company}</div>
                <div style={{display:'flex',alignItems:'center',gap:9,marginTop:6,fontSize:12.5,color:'var(--muted-foreground)',flexWrap:'wrap'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:20,minWidth:0}}>
            ...
            <div className="card card-pad">
              <SectionHead icon="message" title="Meeting history" sub={`${meetings.length} recent · ${p.meetings} all‑time`}
                action={<button className="btn btn-ghost btn-sm" style={{border:'1px solid var(--border-strong)'}} onClick={()=>flash&&flash('Logging a meeting…')}><Icon name="plus" size={14}/>Log</button>}/>
                ...
                {meetings.map((m,i)=>{ const mt=MTG_TYPE[m.type], ch=MTG_CH[m.channel]; return (
                  <div key={i} style={{display:'flex',gap:14,position:'relative'}}>
                    <div style={{display:'flex',flexDirection:'column',alignItems:'center',flex:'none'}}>
                      ...
                      {i<meetings.length-1 && <span style={{width:2,flex:1,background:'var(--border)',minHeight:18}}></span>}
                    </div>
                    <div style={{flex:1,minWidth:0,paddingBottom:i<meetings.length-1?16:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap'}}>
                        <span style={{fontSize:13.5,fontWeight:600,color:'var(--foreground)'}}>{mt.label}</span>
                        <span className="badge" style={{background:'var(--background)',color:'var(--muted-foreground)',height:18,fontSize:10.5,border:'1px solid var(--border)'}}><Icon name={ch.icon} size={11}/>{ch.label}</span>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:6,marginTop:6,fontSize:11.5,color:'var(--muted-foreground)'}}><Avatar id={m.who} size={16}/>with {PEOPLE[m.who].name}</div>
                    </div>
                  </div>
                );})}
              </div>
                  ...
                  {st.tasks.map((t,i)=>{
                    ...
                    return (
                      <div key={t.id} onClick={()=>openTask&&openTask(t.id)}
                        style={{display:'flex',alignItems:'center',gap:12,padding:'11px 8px',margin:'0 -8px',borderTop:i?'1px solid var(--border)':0,cursor:'pointer',borderRadius:8,transition:'.12s'}}
                        onMouseEnter={e=>e.currentTarget.style.background='var(--background)'}
                        onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                        <PriorityFlag k={t.priority}/>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:13,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{t.file}</div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            <div className="card card-pad">
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontSize:13.5,fontWeight:600,color:'var(--foreground)'}}>{p.relType}</div>
              ...
              {owner ? (
                <div style={{display:'flex',alignItems:'center',gap:11,padding:'9px 0',borderTop:'1px solid var(--border)',marginTop:8}}>
                  <Icon name="user" size={15} style={{color:'var(--ink-4)',flex:'none'}}/>
                  <span style={{fontSize:12.5,color:'var(--muted-foreground)',width:74,flex:'none'}}>Owner</span>
                  <span style={{flex:1,display:'flex',alignItems:'center',gap:7,cursor:'pointer'}} onClick={()=>window.__openPerson&&window.__openPerson(owner.id)}>
                    <Avatar id={owner.id} size={20}/><span style={{fontSize:12.5,fontWeight:600,color:'var(--foreground)'}}>{owner.name}</span>
                  </span>
                </div>
              ) : (
              ...
              {/* renewal */}
              {days!=null ? (
                <div style={{paddingTop:12,marginTop:6,borderTop:'1px solid var(--border)'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:7}}>
                    <span style={{fontSize:12.5,color:'var(--muted-foreground)'}}>Contract term</span>
                    <span style={{fontSize:12.5,fontWeight:600,color:p.rel==='renewal'?'var(--orange)':'var(--foreground)'}}>{days}d to renewal</span>
                  </div>
                ...
                <div style={{paddingTop:11,marginTop:6,borderTop:'1px solid var(--border)',fontSize:12.5,color:'var(--muted-foreground)',display:'flex',alignItems:'center',gap:7}}>
            ...
            <div className="card card-pad">
              ...
              <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid var(--border)',fontSize:12.5,color:'var(--secondary-foreground)',lineHeight:1.55}}>{p.bio}</div>
            </div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:2}}>
                  {st.owned.map((f,i)=>(
                    <div key={f.id} onClick={()=>{window.__reviewFolder=f.name;setPage('review');}}
                      style={{display:'flex',alignItems:'center',gap:11,padding:'9px 8px',margin:'0 -8px',borderTop:i?'1px solid var(--border)':0,cursor:'pointer',borderRadius:8,transition:'.12s'}}
                      onMouseEnter={e=>e.currentTarget.style.background='var(--background)'}
                      onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:12.5,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{f.name}</div>
                  ...
                  {st.devices.map((d,i)=>{
                    ...
                    return (
                      <div key={d.id} onClick={()=>openDevice&&openDevice(d.id)}
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12.5,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{d.name}</div>
