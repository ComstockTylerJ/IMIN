...
function Field({label, req, hint, children, bad}){
  return (
    <div>
      <label style={{display:'flex',alignItems:'center',gap:6,fontSize:12.5,fontWeight:600,color:bad?'#DC2626':'var(--secondary-foreground)',marginBottom:7}}>
...
function DataUploadWorkspace({setPage, flash}){
  ...
  const inputStyle={width:'100%',height:40,border:'1px solid var(--border-strong)',borderRadius:9,padding:'0 13px',fontSize:14,fontFamily:'inherit',outline:'none',background:'#fff'};
          ...
          <div style={{display:'flex',flexDirection:'column',gap:18}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:18}}>
                <Field label="Dataset name" req bad={tried&&missing.name}>
                  <input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Northern Terminal — Q2 Throughput"
                    style={{...inputStyle,borderColor:tried&&missing.name?'#DC2626':'var(--border-strong)'}}/>
                </Field>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
                  <Field label="Source system" req bad={tried&&missing.source}>
                    <select value={source} onChange={e=>setSource(e.target.value)} style={{...inputStyle,borderColor:tried&&missing.source?'#DC2626':'var(--border-strong)',color:source?'var(--foreground)':'var(--ink-4)'}}>
                      <option value="">Select a source…</option>
                      {U_SOURCES.map(s=><option key={s} value={s}>{s}</option>)}
                    </select>
                  </Field>
                  <Field label="Owner">
                    <div style={{...inputStyle,display:'flex',alignItems:'center',gap:9,background:'var(--background)'}}>
                      <Avatar id="tyler" size={22}/><span style={{fontSize:13.5,color:'var(--secondary-foreground)',fontWeight:500}}>Tyler Chen (you)</span>
                    </div>
                  </Field>
                </div>
                  ...
                  <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                    {U_CATS.map(c=>(
                      <button key={c.id} onClick={()=>setCat(c.id)} style={{display:'inline-flex',alignItems:'center',gap:7,height:36,padding:'0 14px',borderRadius:9,cursor:'pointer',fontSize:13,fontWeight:550,transition:'.12s',
                        border:'1px solid '+(cat===c.id?'var(--primary)':'var(--border-strong)'),background:cat===c.id?'var(--primary)':'#fff',color:cat===c.id?'#fff':'var(--secondary-foreground)'}}>
                  ...
                  <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                    {U_CLS.map(c=>(
                      <button key={c.id} onClick={()=>setCls(c.id)} style={{display:'inline-flex',alignItems:'center',gap:8,height:36,padding:'0 14px',borderRadius:9,cursor:'pointer',fontSize:13,fontWeight:600,transition:'.12s',
                        border:'1.5px solid '+(cls===c.id?c.color:'var(--border-strong)'),background:cls===c.id?c.color+'12':'#fff',color:cls===c.id?c.color:'var(--muted-foreground)'}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:18}}>
                ...
                <Field label="Description" req bad={tried&&missing.desc}>
                  <textarea value={desc} onChange={e=>setDesc(e.target.value)} placeholder="What is in this dataset, how it was collected, and any limitations…"
                    style={{...inputStyle,height:96,padding:'11px 13px',lineHeight:1.5,resize:'vertical',borderColor:tried&&missing.desc?'#DC2626':'var(--border-strong)'}}/>
                </Field>
                ...
                <Field label="Handling caveats">
                  <div style={{display:'flex',flexDirection:'column',gap:7}}>
                    {U_CAVEATS.map(c=>(
                      <label key={c} style={{display:'flex',alignItems:'center',gap:10,fontSize:13,color:'var(--secondary-foreground)',cursor:'pointer'}}>
                        <span onClick={()=>toggle(caveats,setCaveats,c)} style={{width:18,height:18,borderRadius:5,border:'1.5px solid',borderColor:caveats.includes(c)?'var(--primary)':'var(--border-strong)',background:caveats.includes(c)?'var(--primary)':'#fff',display:'flex',alignItems:'center',justifyContent:'center',flex:'none',transition:'.12s'}}>{caveats.includes(c)&&<Icon name="check" size={11} sw={3} style={{color:'#fff'}}/>}</span>
                        {c}
                      </label>
                    ))}
                  </div>
            ...
            <div className="card card-pad">
              ...
              <div onDragOver={e=>{e.preventDefault();setOver(true);}} onDragLeave={()=>setOver(false)}
                ...
                style={{border:'1.5px dashed '+(over?'var(--blue)':tried&&missing.files?'#DC2626':'var(--border-strong)'),borderRadius:12,padding:'30px 20px',textAlign:'center',cursor:'pointer',
                  background:over?'var(--blue-t)':'var(--background)',transition:'.15s'}}>
                <span style={{width:46,height:46,borderRadius:12,background:'#fff',border:'1px solid var(--border)',color:'var(--blue)',display:'inline-flex',alignItems:'center',justifyContent:'center'}}><Icon name="upload" size={22}/></span>
                <div style={{fontSize:14,fontWeight:600,color:'var(--foreground)',marginTop:11}}>Drag files here, or <span style={{color:'var(--blue)'}}>browse</span></div>
              ...
              {files.length>0 && (
                <div style={{display:'flex',flexDirection:'column',gap:9,marginTop:14}}>
                  {files.map(f=>{
                    ...
                    return (
                      <div key={f.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 13px',border:'1px solid var(--border)',borderRadius:10}}>
                        <span style={{width:34,height:34,borderRadius:9,background:'var(--background)',border:'1px solid var(--border)',color:'var(--muted-foreground)',display:'flex',alignItems:'center',justifyContent:'center',flex:'none'}}><Icon name={f.kind} size={16}/></span>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',justifyContent:'space-between',gap:10}}>
                            <span style={{fontSize:13,fontWeight:550,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{f.name}</span>
          ...
          <div style={{position:'sticky',top:'calc(var(--header-h) + 16px)',display:'flex',flexDirection:'column',gap:14}}>
            <div className="card card-pad">
              ...
              <div style={{display:'flex',flexDirection:'column',gap:2}}>
                {[['Dataset name',!missing.name],['Source system',!missing.source],['Category',!missing.cat],['Classification',!missing.cls],['Description',!missing.desc],['At least one file',!missing.files]].map(([l,ok])=>(
                  <div key={l} style={{display:'flex',alignItems:'center',gap:10,padding:'6px 0',fontSize:13,color:ok?'var(--secondary-foreground)':'var(--muted-foreground)'}}>
              ...
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',fontSize:12.5}}>
                <span className="muted">Files</span><span style={{fontWeight:600,color:'var(--foreground)'}}>{files.length}</span>
              </div>
            ...
            <div className="card card-pad" style={{background:'var(--background)'}}>
              <div style={{display:'flex',gap:10,alignItems:'flex-start'}}>
                <Icon name="shield" size={16} style={{color:'var(--muted-foreground)',flex:'none',marginTop:1}}/>
    ...
    <div className="rise">
      ...
      <div className="page" style={{paddingTop:24,maxWidth:680,margin:'0 auto'}}>
        <div className="card card-pad" style={{textAlign:'center',padding:'40px 28px'}}>
          ...
          <h2 style={{fontSize:22,fontWeight:700,letterSpacing:'-.02em',margin:'18px 0 0',color:'var(--foreground)'}}>Submitted to the ingest queue</h2>
          <p className="sec" style={{fontSize:14,margin:'8px 0 0',lineHeight:1.55}}><b style={{color:'var(--foreground)'}}>{name||'Your dataset'}</b> — {files.length} file{files.length===1?'':'s'} — is now validating against the schema. You'll be notified when it's indexed.</p>
