  ...
  return (
    <div className="card card-pad" onClick={onClick}
      ...
      <div style={{minWidth:0}}>
        <div style={{fontSize:21,fontWeight:700,color:'var(--foreground)',letterSpacing:'-.02em',lineHeight:1}}>{value}</div>
      ...
      <div style={{borderBottom:'1px solid var(--border)',background:'rgba(255,255,255,.65)',backdropFilter:'blur(4px)'}}>
        <div className="page" style={{paddingTop:16,paddingBottom:20}}>
          <div style={{display:'flex',alignItems:'center',gap:7,fontSize:12.5,color:'var(--muted-foreground)',fontWeight:500,marginBottom:16}}>
            ...
            <span style={{color:'var(--secondary-foreground)'}}>My profile</span>
          </div>
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:20,flexWrap:'wrap'}}>
            <div style={{display:'flex',alignItems:'center',gap:18,minWidth:0}}>
              ...
              <div style={{minWidth:0}}>
                ...
                <div style={{fontSize:14,color:'var(--secondary-foreground)',fontWeight:550,marginTop:5}}>{p.role} · {p.dept}</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) 340px',gap:20,alignItems:'start'}}>
                ...
                {myReqs.slice(0,5).map((r,i)=>{ const s=REQ_STATUS[r.status]; return (
                  <div key={r.id} onClick={()=>setPage('requests')}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{r.name}</div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            <div className="card card-pad">
              ...
              <button className="btn btn-ghost btn-sm" style={{border:'1px solid var(--border-strong)',width:'100%',marginTop:12}} onClick={()=>flash&&flash('Opening settings…')}><Icon name="settings" size={14}/>Manage settings</button>
            </div>

            {/* security */}
            <div className="card card-pad">
              <div className="eyebrow" style={{marginBottom:11}}>Security</div>
              <div style={{display:'flex',alignItems:'center',gap:11,fontSize:12.5,color:'var(--secondary-foreground)'}}>
                <Icon name="shield_check" size={16} style={{color:'var(--lime,#16A34A)',flex:'none'}}/>
                <span>Two‑factor authentication is <b style={{color:'var(--foreground)'}}>on</b></span>
              </div>
              <button className="btn btn-ghost btn-sm" style={{border:'1px solid var(--border-strong)',width:'100%',marginTop:12,color:'var(--coral)'}} onClick={()=>flash&&flash('Signed out')}><Icon name="external" size={14}/>Sign out</button>
            </div>
          </div>
        </div>
      </div>
