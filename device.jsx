  ...
  return (
    <div className="card card-pad" style={{display:'flex',alignItems:'center',gap:13}}>
      ...
      <div style={{minWidth:0}}>
        <div style={{fontSize:21,fontWeight:700,color:'var(--foreground)',letterSpacing:'-.02em',lineHeight:1,whiteSpace:'nowrap'}}>{value}</div>
...
function DevInfoRow({icon, label, value, mono, last, accent}){
  return (
    <div style={{display:'flex',alignItems:'center',gap:11,padding:'9px 0',borderBottom:last?0:'1px solid var(--border)'}}>
      <Icon name={icon} size={15} style={{color:'var(--ink-4)',flex:'none'}}/>
      <span style={{fontSize:12.5,color:'var(--muted-foreground)',width:84,flex:'none'}}>{label}</span>
      <span style={{flex:1,minWidth:0,fontSize:12.5,color:accent||'var(--foreground)',fontWeight:550,textAlign:'right',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',fontFamily:mono?'ui-monospace,Menlo,monospace':'inherit'}}>{value}</span>
    </div>
  );
}
      ...
      <div style={{borderBottom:'1px solid var(--border)',background:'rgba(255,255,255,.65)',backdropFilter:'blur(4px)'}}>
        <div className="page" style={{paddingTop:16,paddingBottom:20}}>
          <div style={{display:'flex',alignItems:'center',gap:7,fontSize:12.5,color:'var(--muted-foreground)',fontWeight:500,marginBottom:16}}>
            ...
            <Icon name="chevron_right" size={13} style={{opacity:.6}}/>
            <span style={{color:'var(--secondary-foreground)'}}>{d.name}</span>
          </div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:18,minWidth:0}}>
              ...
              <div style={{minWidth:0}}>
                <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                  <h1 style={{fontSize:25,fontWeight:700,letterSpacing:'-.03em',margin:0,color:'var(--foreground)'}}>{d.name}</h1>
                ...
                <div style={{display:'flex',alignItems:'center',gap:9,marginTop:7,fontSize:12.5,color:'var(--muted-foreground)',flexWrap:'wrap'}}>
                  <span style={{fontFamily:'ui-monospace,Menlo,monospace',fontWeight:600,color:'var(--secondary-foreground)'}}>{d.ev}</span>
            ...
            <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:13}}>
              <div style={{display:'flex',gap:9,flexWrap:'wrap',justifyContent:'flex-end'}}>
                ...
                <div style={{position:'relative'}}>
                  <button className="btn btn-ghost btn-sm" style={{border:'1px solid var(--border-strong)',padding:'0 10px'}} onClick={()=>setMenuOpen(o=>!o)}><Icon name="more" size={16}/></button>
                  {menuOpen && (
                    <React.Fragment>
                      ...
                      <div className="pop card" style={{position:'absolute',top:40,right:0,width:228,padding:6,zIndex:61,boxShadow:'var(--shadow-lg)',borderRadius:11,textAlign:'left'}}>
                        {[
                          ...
                          <button key={it.label} onClick={()=>{setMenuOpen(false);it.fn();}}
                            style={{display:'flex',alignItems:'center',gap:10,width:'100%',padding:'9px 10px',border:0,background:'transparent',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:500,color:'var(--secondary-foreground)',textAlign:'left'}}
                            onMouseEnter={e=>e.currentTarget.style.background='var(--secondary)'}
                            onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                            <Icon name={it.ic} size={15} style={{color:'var(--muted-foreground)',flex:'none'}}/>{it.label}
                          </button>
                        ))}
              ...
              <div style={{display:'flex',alignItems:'center',gap:9}}>
                ...
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:12.5,fontWeight:600,color:'var(--foreground)'}}>Integrity verified</div>
          ...
          {/* specs strip */}
          <div style={{display:'flex',alignItems:'center',gap:'14px 20px',marginTop:18,paddingTop:16,borderTop:'1px solid var(--border)',flexWrap:'wrap'}}>
            {[
              ['Model', d.model, false],
              ['OS', d.os, false],
              ...
              <React.Fragment key={l}>
                {i>0 && <span style={{width:1,height:28,background:'var(--border)',flex:'none'}}></span>}
                <div style={{display:'flex',flexDirection:'column',gap:3,minWidth:0}}>
                  <span style={{fontSize:10,fontWeight:600,letterSpacing:'.07em',textTransform:'uppercase',color:'var(--ink-4)'}}>{l}</span>
                  <span style={{fontSize:12.5,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap',fontFamily:m?'ui-monospace,Menlo,monospace':'inherit'}}>{v}</span>
                </div>
              </React.Fragment>
            ))}
            {/* custodian */}
            <span style={{width:1,height:28,background:'var(--border)',flex:'none'}}></span>
            <div style={{display:'flex',flexDirection:'column',gap:3,minWidth:0}}>
              ...
              <span onClick={()=>openPerson&&openPerson(cust.id)} style={{display:'flex',alignItems:'center',gap:7,cursor:'pointer'}}>
                ...
                <span style={{fontSize:12.5,fontWeight:600,color:'var(--foreground)',whiteSpace:'nowrap'}}>{cust.name}</span>
      ...
      <div className="page" style={{paddingTop:24,display:'flex',flexDirection:'column',gap:20}}>
        ...
        {locked ? (
          <div className="card card-pad" style={{textAlign:'center',padding:'56px 20px'}}>
            ...
            <div style={{fontSize:16,fontWeight:700,color:'var(--foreground)',marginTop:13}}>Device is locked</div>
        ...
        <div onClick={()=>setCocOpen(false)} style={{position:'fixed',inset:0,zIndex:300,background:'rgba(20,32,52,.34)',backdropFilter:'blur(2px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
          <div className="pop card" onClick={e=>e.stopPropagation()} style={{width:460,maxWidth:'100%',maxHeight:'86vh',overflowY:'auto',padding:0,boxShadow:'var(--shadow-lg)',borderRadius:14}}>
            <div style={{display:'flex',alignItems:'center',gap:11,padding:'16px 20px',borderBottom:'1px solid var(--border)'}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:15,fontWeight:700,color:'var(--foreground)'}}>Chain of custody</div>
            ...
            <div style={{padding:'18px 20px'}}>
              {steps.map((s,i)=>(
                <div key={i} style={{display:'flex',gap:14,position:'relative'}}>
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',flex:'none'}}>
                    <span style={{width:32,height:32,borderRadius:'50%',background:s.done?'var(--blue-t)':'var(--background)',color:s.done?'var(--blue)':'var(--ink-4)',display:'flex',alignItems:'center',justifyContent:'center',border:'1px solid '+(s.done?'transparent':'var(--border-strong)'),flex:'none'}}><Icon name={s.done?'check':s.icon} size={15} sw={s.done?2.4:1.75}/></span>
                    {i<steps.length-1 && <span style={{width:2,flex:1,background:s.done?'var(--blue)':'var(--border)',opacity:s.done?.35:1,minHeight:22}}></span>}
                  </div>
                  <div style={{flex:1,paddingBottom:i<steps.length-1?16:0}}>
                    <div style={{display:'flex',alignItems:'center',gap:9,flexWrap:'wrap'}}>
                      <span style={{fontSize:13.5,fontWeight:600,color:'var(--foreground)'}}>{s.label}</span>
                      {!s.done && <span className="badge" style={{background:'var(--background)',color:'var(--ink-4)',height:18,fontSize:10.5,border:'1px solid var(--border-strong)'}}>Pending</span>}
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:6,marginTop:6,fontSize:11.5,color:'var(--muted-foreground)'}}><Avatar id={s.who.id} size={17}/>{s.who.name}</div>
                  </div>
                </div>
              ))}
              <div style={{display:'flex',alignItems:'center',gap:9,marginTop:6,padding:'11px 13px',background:'var(--lime-t)',borderRadius:10}}>
                ...
                <span style={{fontSize:12,color:'var(--secondary-foreground)'}}>Integrity verified — <span style={{fontFamily:'ui-monospace,Menlo,monospace'}}>SHA‑256</span> matches the original acquisition image.</span>
              </div>
            </div>
          </div>
        </div>
